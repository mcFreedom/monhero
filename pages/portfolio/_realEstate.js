...
const PortfolioRealEstate = () => {
  ...
  useEffect(() => {
    setTotal(
      totalForAssets(assetsForCategory(assets, institutions, "realEstate")),
    )
  }, [assets, institutions])

  useEffect(() => {
    const i = institutions
      .filter((i) => i.item.category === "realEstate")
      .map((i) => i.item)
